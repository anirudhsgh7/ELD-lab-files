...
void recp(float complex FFT_input[N], float complex FFT_output[N]) {
    for (int i = 0; i < N; i++) {
        FFT_output[i] = compute_reciprocal(FFT_input[i]);
    }
}
...
int main() {
    ...
    float complex FFT_output_sw[N], FFT_output_hw[N], FFT_recip_sw[N];

    // Software FFT
    XTime_SetTime(0);
    XTime_GetTime(&PS_start_time);
    FFT_stages(FFT_input, FFT_output_sw);
    recp(FFT_output_sw, FFT_recip_sw); // Compute reciprocal
    XTime_GetTime(&PS_end_time);

    // Hardware FFT using DMA
    int status;
    XAxiDma AxiDMA;
    ...
    for (int i = 0; i < N; i++) {
        float diff1 = fabsf(crealf(FFT_recip_sw[i]) - crealf(FFT_output_hw[i]));
        float diff2 = fabsf(cimagf(FFT_recip_sw[i]) - cimagf(FFT_output_hw[i]));

        printf("\nSW Output: %f+%fI, HW Output: %f+%fI",
               crealf(FFT_recip_sw[i]), cimagf(FFT_recip_sw[i]),
